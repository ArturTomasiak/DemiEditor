...
static char* shader_content(const char* location, int32_t arr_limit, _Bool gl_version_fallback);
static int32_t get_uniform_location(Shader* shader, const char* name);

Shader shader_create(const char* vertex_shader_path, const char* fragment_shader_path, int32_t arr_limit, _Bool gl_version_fallback) {
    char* vertex_shader = shader_content(vertex_shader_path, arr_limit, gl_version_fallback);
    char* fragment_shader = shader_content(fragment_shader_path, arr_limit, gl_version_fallback);
    Shader shader = {0};
    shader.cache = NULL;
