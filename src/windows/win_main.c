...
static _Bool gl_version_fallback;
...
int32_t CALLBACK WinMain(
    ...
    Editor editor = editor_create((float)width, (float)height, GetDpiForWindow(hwnd), gl_version_fallback);
    get_editor(&editor);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM w_param, LPARAM l_param) {
    ...
    switch(msg) {
        ...
        case WM_KEYDOWN:
            switch(w_param) {
                case 'V':
                    if (!editor->settings.display && IsClipboardFormatAvailable(CF_UNICODETEXT)) {
                ...
                case VK_LEFT:
                    if (!editor->settings.display)
                        ...
                        settings_key_left(editor);
                break;
                case VK_RIGHT:
                    if (!editor->settings.display)
                        editor_key_right(editor);
                    else
                        settings_key_right(editor);
                break;
                case VK_UP:
                    if (!editor->settings.display)
...
static HGLRC create_context(HDC hdc, HWND hwnd) {
    ...
    if (!hglrc) {
            ...
            HGLRC hglrc = wglCreateContextAttribsARB(hdc, 0, attribs_fallback);
