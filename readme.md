...
![glew.h opened in DemiEditor](glew.png)
...
The project's structure is placeholder until I abstract away the entire editor logic and window management.


```bash
src/
├── platform_layer.h   # platform layer abstraction
├── includes.h         # global includes and macros
├── editor.h           # editor logic and rendering, split into editor.c and settings.c
├── character/         # Character map, character buffer and operations the likes of insert char, delete char, paste...
...
├── windows/           # main function, platform layer and error/debug handling for windows
...
- fallbacks for older windows.h calls at failure
...
Demi is compatible with all hardware that runs windows 10+ and supports opengl 3.3.

With vsync off Demi can modify the contents of glew.h (which contains 26427 lines of code) at around 430-440 frames per second on a very low end laptop.
...
- added settings menu with adjustable:
    ...
    - line spacing
    - scroll speed
- added fallback to opengl 3.3 if initializing 4.6 fails
